...
Item {
  ...
  function updateArgs(kind, agentIds) {
    ...
    for (var id in providers) {
      if (providers[id] && providers[id].enabled === false) args.push("--except", id)
    }
    if (agentIds) {
      for (var i = 0; i < agentIds.length; i++) args.push(agentIds[i])
    }
    return args
  }

  function updateCommand(kind, agentIds) {
    var args = updateArgs(kind, agentIds)
    var grokFlags = ""
    if (kind === "force") grokFlags = " --force"
  ...
  property var enabledProviders: {
    ...
    var preferred = String(root.defaultAgentId || "")
    var result = []
    var localIds = {}
    for (var i = 0; i < agents.length; i++) {
      var record = agents[i] ? agents[i].record : null
    ...
    result.sort(function(a, b) {
      var ra = a.providerId === preferred ? 0 : 1
      var rb = b.providerId === preferred ? 0 : 1
      if (ra !== rb) return ra - rb
      var ha = (a.ready && String(a.usageStatusText || "") === "") ? 0 : 1
      var hb = (b.ready && String(b.usageStatusText || "") === "") ? 0 : 1
